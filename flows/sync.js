...
async function syncFlows() {
    try {
        ...
        const apiKey = settings?.Values?.EXTERNAL_API_KEY;
        if (!apiKey) {
            console.error("Error: EXTERNAL_API_KEY not found in local.settings.json");
            process.exit(1);
        }

        const apiUrl = `https://sanme.azurewebsites.net/api/flows/${apiKey}`;
